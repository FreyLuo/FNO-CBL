...
labels = ['(b-1)', '(b-2)', '(b-3)']
titles = ["u', 120 m", "u', 600 m", "u', 1240 m"]
...
for z_layer_index, z_layer in enumerate(z_layers):
    ...
    if z_layer_index == 0:    
        ...
        for i, time_step in enumerate(time_steps_to_plot):
            ...
            axs[z_layer_index].text(fixed_x + 3., fixed_y - 3, f'{int(time_step * t_delat / 60)}h', color=colors[i], ha='left',  va='bottom')
        
    axs[z_layer_index].set_xlim(0.89, 250)
    axs[z_layer_index].set_ylim(1e-6, 1e2)
    axs[z_layer_index].set_xscale('log')
    axs[z_layer_index].set_yscale('log')
    axs[z_layer_index].set_xlabel(r'$ k_h $', labelpad=-10)
    axs[z_layer_index].set_ylabel(r'$ E_u $', labelpad=0)
    if z_layer_index == 0:
...
plt.tight_layout()
filename_mean = f"figure3b.{file_extension}"
