...
#       fluctuation = selected_data - mean_val[np.newaxis, np.newaxis, :]
#       rms_val = np.sqrt(np.mean(fluctuation**2, axis=(0, 1))).squeeze()
#       skew_val = np.mean(fluctuation**3, axis=(0, 1)).squeeze()
#       plot_val_les[i,:] = skew_val / rms_val **(3)
...
for variable_index in range(0, 1):
    ...
    for i, time_step in enumerate(time_steps_to_plot):
        for j, case in enumerate(cases):
            ...
            rms_val = np.sqrt(np.mean(fluctuation**2, axis=(0, 1))).squeeze()
            skew_val = np.mean(fluctuation**3, axis=(0, 1)).squeeze()
            plot_val[j,i,:] = skew_val / rms_val **(3)
    ...
    for i, time_step in enumerate(time_steps_to_plot):
        ...
        y_pos = y_base - i * 0.18  
        plt.text(fixed_x+0.1, y_pos, 
                 f'{int(time_step * t_delat / 60)}h', 
                 color=colors[i], 
                 ha='left', 
                 va='bottom')
    plt.text(-0.24, 1.02, '(d)', fontsize=46, color='black', ha='left', va='top', transform=plt.gca().transAxes)
    plt.ylabel('z (km)') 
    plt.xlabel(r'$\langle w^{\prime 3} \rangle / \langle w^{\prime 2} \rangle ^{3/2}$')
    plt.ylim(0, 2) 
    plt.xlim(-0.5, 4) 
    ...
    plt.grid(True)
    plt.tight_layout()
    filename_mean = f"figure2d.{file_extension}"
