...
#       selected_data = data[time_step, :, :, :,0]                 
#       mean_val = np.mean(selected_data, axis=(0, 1)).squeeze() 
#       th_fluctuation = selected_data - mean_val[np.newaxis, np.newaxis, :]
      
#       selected_data = data[time_step, :, :, :,3]                 
#       mean_val = np.mean(selected_data, axis=(0, 1)).squeeze() 
#       w_fluctuation = selected_data - mean_val[np.newaxis, np.newaxis, :]
      
#       thw = th_fluctuation  * w_fluctuation
#       print("thw :", thw.shape)        
#       rms_val = np.mean(thw, axis=(0, 1)).squeeze()
#       plot_val_les[i,:] = rms_val / 0.24
...
for variable_index in range(0, 1):
    ...
    for i, time_step in enumerate(time_steps_to_plot):
        for j, case in enumerate(cases):
            selected_data = data[case, time_step, :, :, :,0]                 
            mean_val = np.mean(selected_data, axis=(0, 1)).squeeze() 
            th_fluctuation = selected_data - mean_val[np.newaxis, np.newaxis, :]
    ...
    for i, time_step in enumerate(time_steps_to_plot):
        ...
        y_pos = y_base - i * 0.18 
        plt.text(fixed_x+0.55, y_pos, 
                 f'{int(time_step * t_delat / 60)}h', 
                 color=colors[i], 
                 ha='left', 
                 va='bottom')
    plt.text(-0.24, 1.02, '(c)', fontsize=46, color='black', ha='left', va='top', transform=plt.gca().transAxes)
    plt.ylabel('z (km)')
    plt.xlabel(r'$\langle \theta^{\prime} w^{\prime} \rangle /  Q_s $ ') 
    plt.ylim(0, 2)  
    plt.xlim(-0.25, 1.0)  
    plt.gca().xaxis.set_major_locator(MaxNLocator(nbins=5))  
    plt.tick_params(axis='y', direction='in')
    plt.tick_params(axis='x', direction='in') 
    plt.legend(loc='upper right', bbox_to_anchor=(1.032, 1.032))  
    plt.grid(True)
    plt.tight_layout()
    filename_mean = f"figure2c.{file_extension}"
